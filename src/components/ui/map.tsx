...
function DefaultLoader() {
  // No backdrop-blur: stacking a GPU filter over the MapLibre WebGL canvas
  // compounds GPU load. Plain opacity layer + animated dots is enough.
  return (
...
function MapMarker({
  ...
  // Lazy-init via useRef: useMemo([]) re-invokes its factory under
  // React Strict Mode, producing an orphaned Marker (with DOM listeners
  // attached to a discarded element). useRef.current persists across the
  // double-render so we only ever build one Marker instance.
  ...
  // Consumed via context below, not read into render output; safe under the lazy-init pattern above.
  ...
  // Sync mutable props imperatively via effects (NOT in the render body).
  // Calling setLngLat/setRotation/etc. during render is unsafe under Strict
  // Mode (double-render) and can leave the Marker in an inconsistent state.
  useEffect(() => {
    if (
  ...
  useEffect(() => {
    const nextRotation = markerOptions.rotation ?? 0;
    if (marker.getRotation() !== nextRotation) {
      marker.setRotation(nextRotation);
    }
  }, [marker, markerOptions.rotation]);

  useEffect(() => {
    const nextAlignment = markerOptions.rotationAlignment ?? "auto";
    if (marker.getRotationAlignment() !== nextAlignment) {
      marker.setRotationAlignment(nextAlignment);
    }
  }, [marker, markerOptions.rotationAlignment]);

  useEffect(() => {
    const nextAlignment = markerOptions.pitchAlignment ?? "auto";
    if (marker.getPitchAlignment() !== nextAlignment) {
      marker.setPitchAlignment(nextAlignment);
    }
...
function MarkerPopup({
  ...
  // Lazy-init via ref to avoid orphaned DOM nodes / Popup instances under
  // React Strict Mode (which double-invokes useMemo factories in dev).
